...
if "implicit_corpus" not in os.environ:
    raise KeyError(
        "Please supply the main working directory as environment "
...
if "eval_corpus" not in os.environ:
    raise KeyError(
        "Please supply the exte`rnal data directory as environment "
...
c.PropBankConfig.propfile = os.path.join(
    corpus_path, "propbank-LDC2004T14", "data", "prop.txt"
)
