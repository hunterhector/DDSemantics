...
def main():
    ...
    class EreConf(DataConf):
        ...
        src_ext = Unicode(help="Source file extension", default_value=".xml").tag(
            config=True
        )
        ere_ext = Unicode(help="Ere file extension", default_value=".rich_ere.xml").tag(
            config=True
        )
        ere_split = Bool(help="Whether split ere based on the file names").tag(
            config=True
        )
        ignore_quote = Bool(help="model name", default_value=False).tag(config=True)
        format = Unicode(help="name for format", default_value="ERE").tag(config=False)
    ...
    class AceConf(DataConf):
        ...
        format = Unicode(help="name for format", default_value="ACE").tag(config=True)
    ...
    class NomBankConfig(DataConf):
        ...
        implicit_path = Unicode(help="Implicit annotation xml path.").tag(config=True)
    ...
    class NegraConfig(DataConf):
        data_files = List(help="Input data path.", trait=Unicode).tag(config=True)
        stat_out = Unicode(help="Output statistics").tag(config=True)
        format = Unicode(help="name for format", default_value="Negra").tag(config=True)
    ...
    for doc in first_parser.get_doc():
        ...
        out_path = os.path.join(output_param.brat_dir, "data", doc.docid + ".ann")
        ensure_dir(out_path)
        with open(out_path, "w") as out:
            out.write(ann_text)

        out_path = os.path.join(output_param.brat_dir, "data", doc.docid + ".txt")
        ensure_dir(out_path)
        with open(out_path, "w") as out:
