...
class ClozeSampler:
    ...
    def sample_cross(self, arg_pool, origin_start, origin_end):
        ...
        for arg_info in arg_pool:
            if not (
                arg_info["arg_start"] == origin_start
...
class CandidateBuilder:
    def __init__(
        self, event_emb_vocab: EmbbedingVocab, typed_event_vocab: TypedEventVocab
    ):
...
class ClozeGenerator:
    def __init__(self, candidate_builder: CandidateBuilder, fix_slot_mode: bool = True):
    ...
    def cross_cloze(self, arg_list, doc_args, target_arg_idx):
        ...
        for idx, (slot, content) in enumerate(arg_list):
            if idx == target_arg_idx:
                neg_instance.append(
                    (slot, self.builder.build_candidate(content, sample_res))
                )
            else:
    ...
    def inside_cloze(self, arg_list, arg_index):
        ...
        sample_indexes = list(range(arg_index)) + list(range(arg_index, len(arg_list)))
        ...
        for idx, (slot, content) in enumerate(arg_list):
            if idx == arg_index:
                ...
                neg_instance.append(
                    (
                        slot,
                        self.builder.build_candidate(base_slot, swap_slot_info, True),
                    )
                )
            elif idx == swap_index:
                ...
                neg_instance.append(
                    (
                        slot,
                        self.builder.build_candidate(base_slot, origin_slot_info, True),
                    )
                )
            else:
                neg_instance.append((slot, content))
...
class TestClozeMaker:
    ...
    def gen_test_args(self, test_stub, doc_args, pred_sent, distance_cap=float("inf")):
        ...
        for doc_mention in doc_args:
            ...
            update_arg = self.builder.build_candidate(test_stub, doc_mention, True)
