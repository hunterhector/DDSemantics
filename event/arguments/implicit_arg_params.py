...
class ArgModelPara(Configurable):
    # Basic configs.
    use_gpu = Bool(help="Whether to use gpu.", default_value=True).tag(config=True)
    model_type = Unicode(help="Type of the model.", default_value="").tag(config=True)

    # Input size configs.
    event_arg_vocab_size = Int(help="Vocabulary size of events and argument words").tag(
        config=True
    )
    ...
    word_embedding_dim = Int(help="Dimension of word embedding", default_value=300).tag(
        config=True
    )

    # Training controls
    early_stop_patience = Int(help="Early stop patience", default_value=1).tag(
        config=True
    )
    ...
    multi_context = Bool(
        help="Whether to use only one context, " "or multiple context per document"
    ).tag(config=True)
    max_events = Int(
    ...
    max_cloze = Int(
        help="Maximum number of cloze to extract per document", default_value=150
    ).tag(config=True)

    # Model architecture related parameters.
    loss = Unicode(
        help="Loss type for implicit argument training", default_value="cross_entropy"
    ).tag(config=True)
    ...
    arg_representation_method = Unicode(
        help="The method used to represent the argument slots."
    ).tag(config=True)
    ...
    event_composition_layer_sizes = List(
        Int,
        default_value=[400, 200],
        help="Output size of the event composition layers.",
    ).tag(config=True)

    event_composition_activation = Unicode(default_value="tanh").tag(config=True)
    slot_reduction_activation = Unicode(default_value="tanh").tag(config=True)

    transformer_dim = Int(default_value=50).tag(config=True)
    ...
    encode_distance = Unicode(help="Method to encode distances").tag(config=True)
    ...
    vote_pooling = Unicode(
        help="Method to pool the multiple votes.", default_value="kernel"
    ).tag(config=True)

    pool_topk = Int(
        help="The K value if Top K pooling is enabled.", default_value=3
    ).tag(config=True)

    # Null Instantiation Detector.
    nid_method = Unicode(
        help="The method for Null Instantiation Detector", default_value="logistic"
    ).tag(config=True)
    use_ghost = Bool(
    ...
    # Other parameters.
    use_gold_mention = Bool(help="Whether to use gold mentions.").tag(config=True)
    use_auto_mention = Bool(help="Whether to use auto mentions.").tag(config=True)

    distance_cap = Int(help="Max distance from current in test.", default_value=3).tag(
        config=True
    )

    # Baseline parameters.
    w2v_baseline_method = Unicode(help="Baseline method type.", default_value="").tag(
        config=True
    )
    w2v_baseline_avg_topk = Int(help="Average top K", default_value=3).tag(config=True)
    w2v_event_repr = Unicode(
        help="Methods for creating the embedding.", default_value=""
    ).tag(config=True)

    gold_role_field = Unicode(help="Field name for the gold standard").tag(config=True)
